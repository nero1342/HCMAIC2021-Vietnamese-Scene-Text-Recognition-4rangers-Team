...
def load_zip_file(file, fileNameRegExp="", allEntries=False):
    ...
    for name in archive.namelist():
        ...
        if fileNameRegExp != "":
            m = re.match(fileNameRegExp, "/".join(name.split('/')[1:]))
            if m == None:
                addFile = False
            else:
                if len(m.groups()) > 0:
        ...
        # print(name, fileNameRegExp, addFile)
        if addFile:
            # print(keyName, name)
