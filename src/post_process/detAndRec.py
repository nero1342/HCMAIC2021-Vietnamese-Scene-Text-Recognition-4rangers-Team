...
def default_evaluation_params():
    ...
    return {
        ...
        "ONLY_REMOVE_FIRST_LAST_CHARACTER": False,
    }
    ...
    for resFile in gt:
        ...
        if resFile in subm:
            ...
            for n in range(len(pointsList)):
                ...
                transcription = transcriptionsList[n]
                try:
                    transcription, scores = transcription.split('|')
                except:
                    scores = 0
                    # print(transcriptionsList[n], resFile) 
                    # exit(0)
            
                # transcription = transcription.replace("D-", "Đ")
                if evaluationParams["LTRB"]:
            ...
            if len(gtPols) > 0 and len(detPols) > 0:
                ...
                for detNum in range(len(detPols)):
                                ...
                                try:
                                    ...
                                    if not correct: 
                                        ...
                                        def checka(a, b):
                                            if len(a) != len(b):
                                                return 100
                                            cnt = [a[i] != b[i] for i in range(len(a))].count(True)
                                            # print(a, b, cnt)
                                            return cnt 
                                        if checka(detTrans[detNum].upper(), gtTrans[gtNum].upper()) < 2:
                                            # correct = True
                                            totalWrong += 1
                                            # print(f" ---> Det {int(resFile):04d}-{totalWrong:04d}", detTrans[detNum].upper(), gtTrans[gtNum].upper(), detScore[detNum])
                                        # WRONG.add(gtTrans[gtNum].upper())
                                except:  # empty
                                    correct = False
                                    cnt += 1
                                    totalWrong += 1
                                    # print(" ---> Det2", totalWrong, detTrans[detNum].upper(), gtTrans[gtNum].upper(), correct)
                    ...
                    # if gtRectMat[gtNum] == 1:
                    #     # print("Plus one", gtTrans[gtNum].upper())
                    #     WRONG.add(gtTrans[gtNum].upper())       
                # if cnt > 0:
                #     print('resgt', resFile, cnt) 
        ...
        # for detNum in range(len(detPols)):
        #     if ( det_only_detRectMat[detNum] == 0 ):
        #         print(f" ---> Det {int(resFile):04d}-{totalWrong:04d}", detTrans[detNum].upper(), detScore[detNum])
...
def text_eval_main(det_file, gt_file, is_word_spotting=False):
    global WORD_SPOTTING
